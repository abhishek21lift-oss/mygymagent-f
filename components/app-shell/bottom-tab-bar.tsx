...
function TabLink({ tab, active }: { tab: TabItem; active: boolean }) {
  const Icon = tab.icon;
  return (
    <Link href={tab.href} aria-current={active ? "page" : undefined} className={cn(
      "relative flex min-h-14 flex-1 flex-col items-center justify-center gap-0.5 touch-manipulation rounded-xl transition-all duration-200 focus-visible:outline-2 focus-visible:outline-ring",
      active ? "bg-gradient-to-b from-primary/12 to-transparent font-semibold text-primary" : "font-medium text-muted-foreground hover:bg-muted/50 hover:text-foreground",
    )}>
      {active && <span aria-hidden="true" className="absolute top-1 h-1 w-7 rounded-full bg-gradient-to-r from-primary via-violet-500 to-cyan-400" />}
...
export function BottomTabBar({ onOpenMore }: { onOpenMore: () => void }) {
  ...
  return (
    <nav aria-label="Primary" className="fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-background/80 pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_30px_rgba(0,0,0,0.06)] backdrop-blur-2xl md:hidden">
      <div className="relative mx-auto flex max-w-lg items-stretch justify-between gap-1 px-2 py-1.5">
        ...
        <Link href="/ai" aria-label="AI Agent" aria-current={aiActive ? "page" : undefined} className="relative -mt-6 flex flex-1 touch-manipulation flex-col items-center justify-end gap-1 rounded-xl pb-1 focus-visible:outline-2 focus-visible:outline-ring">
          <span className={cn("flex size-14 items-center justify-center rounded-full border shadow-lg transition-transform", aiActive ? "scale-105 border-violet-400 bg-gradient-to-br from-violet-500 via-fuchsia-500 to-cyan-400 text-white" : "border-border bg-gradient-to-br from-violet-500 via-fuchsia-500 to-cyan-400 text-white")}>
