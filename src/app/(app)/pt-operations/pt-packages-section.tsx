...
function SellPackageDialog() {
 ...
 <Button className="rounded-lg"><Plus className="size-4" /> Sell package</Button>
 </DialogTrigger>
 <DialogContent>
...
export function PtPackagesSection() {
 ...
 {packages.isError ? (
 <div className="rounded-lg border border-rose-200 bg-rose-50 p-5 text-sm font-semibold text-rose-700">Unable to load PT packages.</div>
 ) : packages.isLoading ? (
 <div className="h-20 animate-pulse rounded-lg bg-stone-100" />
 ) : items.length === 0 ? (
 <p className="rounded-lg border border-dashed border-stone-200 bg-stone-50/70 p-8 text-center text-sm font-medium text-stone-500">No PT packages sold yet. Sell the first pack to start tracking session allowances.</p>
 ) : (
