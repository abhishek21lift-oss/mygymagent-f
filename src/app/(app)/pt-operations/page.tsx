...
function Metric({ icon: Icon, label, value, hint, tone }: { icon: typeof Users; label: string; value: React.ReactNode; hint?: string; tone: keyof typeof tones }) {
 return <Card className="group relative overflow-hidden border-white/90 bg-card shadow-[0_20px_60px_-38px_rgba(79,70,229,.35)] transition hover:-translate-y-1 hover:shadow-[0_28px_70px_-38px_rgba(79,70,229,.42)]"><div className="pointer-events-none absolute -right-8 -top-8 size-28 rounded-full bg-primary/10 blur-2xl" /><CardContent className="relative flex items-center gap-4 p-5"><span className={`flex size-14 shrink-0 items-center justify-center rounded-lg shadow-sm ${tones[tone]}`}><Icon className="size-6" /></span><div className="min-w-0"><p className="text-xs font-black uppercase tracking-[.18em] text-stone-400">{label}</p><p className="mt-1 text-2xl font-black tracking-tight text-stone-950">{value}</p>{hint ? <p className="mt-1 text-xs font-medium text-stone-400">{hint}</p> : null}</div></CardContent></Card>;
}
...
export default function PtoOperationsPage() {
 ...
 <section className="grid gap-5"><Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-white px-5 py-5"><CardTitle className="flex items-center gap-3 font-semibold text-xl text-stone-950"><span className="flex size-10 items-center justify-center rounded-xl bg-muted/40 text-violet-700"><Zap className="size-4" /></span>Active coaching queue</CardTitle></CardHeader><CardContent className="p-3 sm:p-4">{assignmentsQuery.isError ? <div className="rounded-lg border border-rose-200 bg-rose-50 p-5 text-sm font-semibold text-rose-700">Unable to load PT assignments. Refresh and try again.</div> : active.length === 0 && !assignmentsQuery.isLoading ? <div className="rounded-lg border border-dashed border-stone-200 bg-stone-50/70 p-10 text-center"><ClipboardCheck className="mx-auto size-8 text-stone-400" /><p className="mt-3 text-sm font-bold text-stone-900">No active PT assignments</p><p className="mt-1 text-xs text-stone-500">Assign a workout plan to begin.</p><Button asChild className="mt-4 rounded-xl"><Link href="/workouts">Assign a workout</Link></Button></div> : <div className="flex flex-col gap-3">{active.map(x => <AssignmentRow key={x.id} assignment={x} />)}</div>}</CardContent></Card></section>
 <PtPackagesSection />
 <Card className="overflow-hidden border-white/90 bg-card shadow-lg "><CardHeader><CardTitle className="font-semibold text-xl text-stone-950">Recent completions</CardTitle></CardHeader><CardContent>{completed.length === 0 ? <p className="text-sm font-medium text-stone-500">No completed assignments yet.</p> : <div className="flex flex-col gap-3">{completed.slice(0,10).map(x => <AssignmentRow key={x.id} assignment={x} />)}</div>}</CardContent></Card>
 </div></div>;
}
