...
export default function MembersPage() {
  ...
  return <div className="flex flex-col gap-7">
    <PageHeader title="Members" description="Understand your members, their lifecycle, and what needs attention." actions={hasPermission("members.create") && <Button className="rounded-xl" onClick={() => router.push("/members/new")}><Plus /> New member</Button>} />
    <section className="relative overflow-hidden rounded-3xl border border-primary/15 bg-gradient-to-br from-primary/[0.09] via-card to-card p-6 shadow-sm">
      ...
      <div className="relative grid gap-5 lg:grid-cols-[1.5fr_1fr] lg:items-end">
        <div><div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-background/70 px-3 py-1.5 text-xs font-semibold text-primary"><Sparkles className="size-3.5" /> Member intelligence</div><h2 className="text-2xl font-semibold tracking-tight">Your member base, in context.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Search members quickly, then open Member 360 for membership, engagement, training and AI insights.</p></div>
        <div className="grid grid-cols-3 gap-2"><MiniMetric icon={Users} label="Showing" value={memberCount} /><MiniMetric icon={AlertTriangle} label="At risk" value="—" /><MiniMetric icon={Clock3} label="Expiring" value="—" /></div>
      </div>
    </section>
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <Segment href="/members" title="All members" description="Browse the complete member base." icon={Users} />
      <Segment href="/members" title="At risk" description="Members whose engagement is falling." icon={AlertTriangle} />
      <Segment href="/members" title="Expiring soon" description="Prioritize upcoming renewals." icon={Clock3} />
      <Segment href="/ai" title="Ask AI" description="Find high-value or inactive members." icon={Sparkles} />
    </section>
    <Card className="overflow-hidden border-0 shadow-sm ring-1 ring-border/70"><CardContent className="p-0"><DataTable columns={columns} data={membersQuery.data} isLoading={membersQuery.isLoading} isError={membersQuery.isError} onRetry={() => membersQuery.refetch()} onRowClick={(member) => router.push(`/members/${member.id}`)} page={page} onPageChange={setPage} search={search} onSearchChange={(value) => { setSearch(value); setPage(1); }} searchPlaceholder="Search by name, email, phone, or member code..." emptyTitle="No members yet" emptyDescription="Add your first member to get started." emptyAction={hasPermission("members.create") ? <Button size="sm" onClick={() => router.push("/members/new")}><UserRound /> Add a member</Button> : undefined} /></CardContent></Card>
  </div>;
}
