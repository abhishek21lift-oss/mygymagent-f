...
export function LeadSelectionStep({ onSelectLead, onSkip }: LeadSelectionStepProps) {
 ...
 <div className="rounded-xl border border-violet-100/70 bg-muted/40 p-4">
 <p className="flex items-center gap-2 text-xs font-black uppercase tracking-[.18em] text-violet-700">
 ...
 <Button variant="outline" className="mt-4 min-h-11 rounded-lg" onClick={onSkip}>
 ...
 : "border-stone-200/70 bg-card hover:border-violet-200"
 }`}
 ...
 <span className={`flex size-10 shrink-0 items-center justify-center rounded-lg font-black text-white shadow-md ${selectedLeadId === lead.id ? "bg-violet-600" : "bg-stone-400"}`} aria-hidden="true">
 ...
 <Button variant="ghost" onClick={onSkip} className="min-h-11 rounded-lg font-bold text-stone-600 hover:text-stone-950">
