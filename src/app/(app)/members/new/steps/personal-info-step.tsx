...
export function PersonalInfoStep({ defaultValues, onUpdate, onContinue }: PersonalInfoStepProps) {
 ...
 setCameraError( "Unable to access the camera. Please allow camera permission and make sure a camera is connected."
 )
 stopCamera()
 } finally {
 setCameraLoading(false)
 ...
 <div className="flex flex-col items-center gap-4 rounded-xl border border-violet-100/70 bg-muted/40 p-6">
 <div className="relative">
 {photoPreview ? (
 <img
 src={photoPreview}
 alt="Profile"
 className="size-24 rounded-xl border-2 border-white object-cover shadow-lg shadow-violet-500/25 ring-2 ring-violet-200"
 />
 ) : (
 <div className="flex size-24 items-center justify-center rounded-xl border-2 border-dashed border-violet-200 bg-card shadow-sm">
 ...
 className="absolute -bottom-1 -right-1 size-9 min-h-0 rounded-full text-sm"
 >
 ...
 <label>
 <Button type="button" variant="outline" size="sm" className="min-h-11 rounded-lg" asChild>
 ...
 {cameraError && (
 <div className="mx-4 mt-4 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-medium text-rose-700" role="alert">
 {cameraError}
 </div>
 )}

 <div className="flex items-center justify-center gap-3 p-4">
 <Button type="button" variant="outline" className="min-h-11 rounded-lg" onClick={() => void switchCamera()}>
 <RefreshCw className="mr-2 size-4" aria-hidden="true" />
 Switch Camera
 </Button>
 <Button type="button" size="lg" className="min-h-11 rounded-lg" onClick={capturePhoto} disabled={!!cameraError}>
