...
export function OnboardingWizard() {
 ...
 <span className="ml-auto hidden rounded-full bg-violet-600 px-3 py-1 font-mono text-xs font-black text-white tabular-nums shadow-md sm:inline-block">
 {Math.round(((currentStep + 1) / STEPS.length) * 100)}%
 </span>
 </div>
 <div className="mt-3 h-2 overflow-hidden rounded-full bg-stone-100" role="presentation">
 <div
 className="h-full rounded-full bg-violet-600 transition-all duration-500 motion-safe:animate-none"
 ...
 {idx < STEPS.length - 1 && (
 <span
 className={`mt-4 h-1 min-w-2 flex-1 rounded-full sm:mx-1 ${idx < currentStep ? "bg-violet-600" : "bg-stone-200"}`}
 aria-hidden="true"
 />
 )}
 </React.Fragment>
