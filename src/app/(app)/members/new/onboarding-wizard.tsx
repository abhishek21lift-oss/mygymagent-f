...
export function OnboardingWizard() {
 ...
 return (
  <div className="mx-auto flex w-full max-w-3xl flex-col gap-6">
   <section aria-label="Onboarding progress" className="overflow-hidden rounded-xl border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl">
     ...
     {STEPS.map((step, idx) => (
      <React.Fragment key={step.id}>
       <li className="flex min-w-0 flex-1 flex-col items-center gap-1.5 text-center" aria-current={idx === currentStep ? "step" : undefined}>
        ...
        <span className={`truncate text-xs font-extrabold ${idx === currentStep ? "text-violet-800" : idx < currentStep ? "text-stone-700" : "text-stone-400"}`}>{step.label}</span>
       </li>
       {idx < STEPS.length - 1 && (
        <span
   ...
   {/* Step Content */}
   <section aria-label={`Step ${currentStep + 1}: ${STEPS[currentStep].label}`} className="min-h-[400px] overflow-hidden rounded-xl border-white/90 bg-white/88 p-5 shadow-xl shadow-violet-900/5 backdrop-blur-xl sm:p-7">
    {currentStep === 0 && (
     <LeadSelectionStep
      onSelectLead={(lead) => {
