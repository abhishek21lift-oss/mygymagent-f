  ...
  <div className="relative -mx-2 min-h-full overflow-hidden pb-12 sm:-mx-3 lg:-mx-5">
   ...
   <div className="mx-auto flex max-w-[1680px] flex-col gap-8 px-2 sm:px-4 lg:px-6">
    ...
    <section aria-labelledby="calendar-board-title" className="grid gap-5 xl:grid-cols-[1.6fr_1fr]">
     <Card className="overflow-hidden rounded-xl border-white/90 bg-white/88 shadow-xl shadow-rose-900/5 backdrop-blur-xl">
      <div className="flex flex-col gap-3 border-b border-stone-100/80 bg-gradient-to-r from-rose-50/90 via-white to-cyan-50/60 px-5 py-5 lg:flex-row lg:items-center lg:justify-between">
       <div className="flex items-center gap-3">
        <span className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-rose-500 to-orange-500 text-white shadow-md shadow-rose-500/25" aria-hidden="true">
         ...
         <div className="ml-2 flex items-center gap-1 rounded-lg border p-1" role="group" aria-label="Calendar view">
          {(["day", "week", "month"] as const).map((v) => (
           <Button
            ...
            onClick={() => setView(v)}
            aria-pressed={view === v}
            className="min-h-10 rounded-md px-3 text-xs font-semibold capitalize"
           >
            {v}
           </Button>
          ))}
      ...
      <CardContent className="p-3 sm:p-5">
       {feed.isLoading ? (
        <div className="space-y-2" aria-label="Loading calendar">
         {[1, 2, 3].map((i) => (
          <div key={i} className="h-16 animate-pulse rounded-lg bg-gradient-to-r from-stone-100 to-stone-50" />
         ))}
        </div>
       ) : feed.isError ? (
  ...
  return (
   <>
    <Button
     ...
     className="h-auto min-h-11 w-full justify-start rounded-lg p-3 text-left text-sm"
    >
...
function MonthGrid({ slots, anchor }: { slots: CalendarSlot[]; anchor: Date }) {
 ...
 return (
  <div className="grid grid-cols-7 gap-1.5">
   {DAYS.map((d) => <div key={d} className="pb-1 text-center text-xs font-black uppercase tracking-wider text-stone-500">{d}</div>)}
   {cells.map((cell, i) =>
    cell === null ? <div key={`pad-${i}`} className="min-h-20 rounded-xl bg-stone-50/40" aria-hidden="true" /> : (
     <div key={`day-${i}`} className="min-h-20 rounded-xl border border-stone-100 bg-white/60 p-1.5 transition hover:border-orange-200 hover:shadow-sm">
      <p className="px-1 text-xs font-black text-stone-500 tabular-nums">{i - lead + 1}</p>
      <div className="flex flex-col gap-1">
       {cell.slice(0, 3).map((s) => {
        ...
        return (
         <div key={s.id + s.startTime} className={`flex items-center gap-1 truncate rounded-md px-1.5 py-0.5 text-xs font-bold ring-1 ${pill}`} title={s.title}>
       ...
       {cell.length > 3 ? <p className="px-1 text-xs font-bold text-stone-500">+{cell.length - 3} more</p> : null}
      </div>
     </div>
    ),
   )}
 ...
 return (
  <Card className="overflow-hidden rounded-xl border-white/90 bg-white/88 shadow-xl shadow-rose-900/5 backdrop-blur-xl">
   <div className="flex items-start gap-3 border-b border-stone-100/80 bg-gradient-to-r from-orange-50/90 via-white to-rose-50/60 px-5 py-5">
    <span className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-orange-500 to-rose-500 text-white shadow-md shadow-orange-500/25" aria-hidden="true">
   ...
   <CardContent className="pt-5">
    <form className="flex flex-col gap-4" onSubmit={submit}>
     ...
     <div className="rounded-xl border border-cyan-100 bg-gradient-to-br from-cyan-50/80 to-blue-50/50 p-3 text-xs text-cyan-900">
       ...
       {staffId ? (
         slots.data?.windows?.length ? (
          slots.data.windows.flatMap((w) => w.free).slice(0, 4).map((f) => (
           <Button key={f.start} type="button" variant="outline" size="sm" className="min-h-10 rounded-md text-xs"
...
function AvailabilityPanel({ branchId }: { branchId: string }) {
 ...
 return (
  <Card className="overflow-hidden rounded-xl border-white/90 bg-white/88 shadow-xl shadow-cyan-900/5 backdrop-blur-xl">
   <div className="flex items-start gap-3 border-b border-stone-100/80 bg-gradient-to-r from-cyan-50/90 via-white to-orange-50/60 px-5 py-5">
    <span className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-500/25" aria-hidden="true">
    ...
    <div className="flex flex-col gap-1.5">
     {(rules.data ?? []).length === 0 ? <p className="text-xs font-medium text-stone-600">No rules yet.</p> : (rules.data ?? []).map((r) => (
      <div key={r.id} className="flex items-center justify-between gap-2 rounded-xl border border-cyan-100 bg-gradient-to-r from-cyan-50/70 to-blue-50/50 px-3 py-2 text-xs">
       ...
       <Button type="button" variant="ghost" size="icon" aria-label="Delete rule" className="min-h-10 min-w-10 rounded-md" onClick={() => guard(() => delRule.mutateAsync(r.id), "Rule removed")}><Trash2 className="size-4" aria-hidden="true" /></Button>
      </div>
     ))}
    </div>
