...
export default function CalendarPage() {
 ...
 <div role="alert" className="rounded-lg border border-rose-200 bg-muted/40 p-5 text-sm font-semibold text-rose-700">Unable to load the calendar feed.</div>
 ) : visible.length === 0 ? (
 <div className="rounded-lg border border-dashed border-rose-200 bg-muted/40 p-10 text-center">
 <span className="mx-auto flex size-12 items-center justify-center rounded-full bg-rose-500 text-white shadow-lg shadow-rose-500/25" aria-hidden="true">
...
function SlotRow({ slot, showDate }: { slot: CalendarSlot; showDate: boolean }) {
 ...
 <DialogHeader>
 <DialogTitle className="font-semibold">{slot.title}</DialogTitle>
 </DialogHeader>
...
function MonthGrid({ slots, anchor }: { slots: CalendarSlot[]; anchor: Date }) {
 ...
 <div key={`day-${i}`} className="min-h-20 rounded-xl border border-stone-100 bg-card p-1.5 transition hover:border-orange-200 hover:shadow-sm">
...
function AvailabilityPanel({ branchId }: { branchId: string }) {
 ...
 <div key={r.id} className="flex items-center justify-between gap-2 rounded-xl border border-cyan-100 bg-muted/40 px-3 py-2 text-xs">
