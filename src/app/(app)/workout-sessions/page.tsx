...
import { CheckCircle2, Dumbbell, Play, Sparkles } from "lucide-react"
...
import { useCompleteWorkoutSession, useStartWorkoutSession, useTodayWorkoutSessions, useWorkoutSession } from "@/lib/hooks/use-workout-sessions"
    ...
    <div className="flex flex-col gap-7">
      ...
      <section className="relative overflow-hidden rounded-3xl border border-primary/15 bg-gradient-to-br from-primary/[0.10] via-card to-card p-6 shadow-sm sm:p-8">
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-background/70 px-3 py-1.5 text-xs font-semibold text-primary"><Sparkles className="size-3.5" /> Execution layer</div>
            <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Coach the workout, not just the plan.</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Start an assigned workout, capture sets as they happen, and finish with a durable execution record for Member 360 and future AI analysis.</p>
          </div>
          <Dumbbell className="hidden size-10 text-primary/60 sm:block" />
        </div>
      </section>

      <div className="grid gap-4 xl:grid-cols-[1fr_1.25fr]">
        <Card className="border-0 shadow-sm ring-1 ring-border/70">
          <CardHeader className="border-b bg-muted/20"><div className="flex items-center justify-between"><div><CardTitle className="text-base">Today's execution queue</CardTitle><p className="mt-1 text-xs text-muted-foreground">Only tenant-scoped sessions returned by the backend.</p></div><Badge variant="outline">{sessions.data?.length ?? 0} sessions</Badge></div></CardHeader>
