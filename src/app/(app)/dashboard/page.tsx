    ...
    <div className="flex flex-col gap-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-2xl">
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-background/70 px-3 py-1.5 text-xs font-semibold text-primary backdrop-blur">
              ...
              MyGymAgent intelligence
            </div>
            <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">
              Your gym, at a glance.
            </h1>
            <p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">
              Start with what needs your attention, then drill into the numbers and take action.
            </p>
          </div>
          <Link
            href="/ai"
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          <MetricCard icon={CalendarCheck} label="Today's check-ins" value={data?.today.checkIns} loading={briefing.isLoading} accent="cyan" />
          <MetricCard icon={Wallet} label="Net revenue" value={data ? `${currency} ${revenue}` : undefined} loading={briefing.isLoading} accent="green" hint="Current period" />
          <MetricCard icon={Users} label="Members at risk" value={data?.atRiskMembers.count} loading={briefing.isLoading} accent="amber" hint="14+ days inactive" />
          <MetricCard icon={Sparkles} label="AI actions" value={data?.pendingAiActions} loading={briefing.isLoading} accent="violet" hint="Awaiting approval" />
        </div>
      </section>
      ...
      <section className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
          ...
          <CardContent className="p-3 sm:p-4">
            {briefing.isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed p-8 text-center">
                ...
                <p className="mt-1 text-xs text-muted-foreground">No urgent operational signals right now.</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-0 bg-gradient-to-br from-violet-500/[0.08] via-card to-card shadow-sm ring-1 ring-primary/15">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base"><Sparkles className="size-4 text-primary" /> AI briefing</CardTitle>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <Link href="/ai" className="inline-flex w-full items-center justify-center gap-2 rounded-xl border bg-background/70 px-4 py-2.5 text-sm font-semibold transition hover:bg-background">
              ...
              <Link key={href} href={href}>
                <Card className="group h-full border-0 shadow-sm ring-1 ring-border/70 transition hover:-translate-y-0.5 hover:ring-primary/25 hover:shadow-md">
                  <CardContent className="flex items-center gap-3 p-4">
                    ...
                    <span className="min-w-0 flex-1"><span className="block text-sm font-semibold">{title}</span><span className="mt-0.5 block truncate text-xs text-muted-foreground">{description}</span></span>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        <Link href="/members" className="rounded-2xl border bg-card p-4 transition hover:-translate-y-0.5 hover:shadow-sm"><div className="flex items-center gap-2 text-sm font-semibold"><Users className="size-4 text-primary" /> Member health</div><p className="mt-1 text-xs text-muted-foreground">Explore retention, attendance and member lifecycle.</p></Link>
        <Link href="/crm" className="rounded-2xl border bg-card p-4 transition hover:-translate-y-0.5 hover:shadow-sm"><div className="flex items-center gap-2 text-sm font-semibold"><Megaphone className="size-4 text-primary" /> Sales pipeline</div><p className="mt-1 text-xs text-muted-foreground">Prioritize leads and follow-ups with context.</p></Link>
        <Link href="/billing" className="rounded-2xl border bg-card p-4 transition hover:-translate-y-0.5 hover:shadow-sm"><div className="flex items-center gap-2 text-sm font-semibold"><CreditCard className="size-4 text-primary" /> Cash flow</div><p className="mt-1 text-xs text-muted-foreground">Outstanding balances and payment activity.</p></Link>
      </section>
    </div>
  );
}

function MetricCard({ icon: Icon, label, value, loading, accent, hint }: { icon: typeof Users; label: string; value?: string | number; loading: boolean; accent: "cyan" | "green" | "amber" | "violet"; hint?: string }) {
  const accents = { cyan: "bg-cyan-500/10 text-cyan-600", green: "bg-emerald-500/10 text-emerald-600", amber: "bg-amber-500/10 text-amber-600", violet: "bg-violet-500/10 text-violet-600" };
  return <Card className="border-0 shadow-sm ring-1 ring-border/70"><CardContent className="p-4"><div className="flex items-start justify-between gap-3"><span className={`flex size-10 items-center justify-center rounded-xl ${accents[accent]}`}><Icon className="size-4.5" /></span>{hint && <span className="text-[11px] text-muted-foreground">{hint}</span>}</div>{loading ? <div className="mt-4 h-7 w-24 animate-pulse rounded bg-muted" /> : <p className="mt-4 text-2xl font-semibold tracking-tight tabular-nums">{value ?? "—"}</p>}<p className="mt-1 text-xs font-medium text-muted-foreground">{label}</p></CardContent></Card>;
}

function MiniInsight({ icon: Icon, label, value }: { icon: typeof TrendingUp; label: string; value: string | number }) {
  return <div className="rounded-xl border bg-background/60 p-3"><div className="flex items-center gap-1.5 text-[11px] text-muted-foreground"><Icon className="size-3.5" />{label}</div><p className="mt-1 text-lg font-semibold tabular-nums">{value}</p></div>;
}
