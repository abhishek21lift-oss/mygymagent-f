  ...
  <Card className={`group relative overflow-hidden border-white/90 bg-white/85 shadow-[0_20px_60px_-38px_rgba(79,70,229,.35)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 ${t.ring}`}>
   ...
   <CardContent className="relative p-5">
    <div className="flex items-start justify-between gap-4">
     <span className={`flex size-11 items-center justify-center rounded-lg bg-gradient-to-br text-white shadow-lg transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-3 ${t.tile}`}>
      <Icon className="size-5" aria-hidden="true" />
     </span>
     {trend ? <Badge variant="secondary" className="rounded-full bg-white/80 px-2.5 py-1 text-xs font-bold ring-1 ring-stone-200/60">{trend}</Badge> : null}
    </div>
    <div className="mt-5">
...
function TrainerList({ trainers }: { trainers: TrainerWorkload[] | undefined }) {
 ...
 return <div className="flex flex-col gap-2">{trainers.map((trainer) => <div key={trainer.trainerId} className="flex items-center justify-between gap-3 rounded-xl border border-stone-100 bg-white/80 p-3"><div className="flex min-w-0 items-center gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white shadow-md"><Dumbbell className="size-4" aria-hidden="true" /></div><div className="min-w-0"><p className="truncate text-sm font-bold text-stone-900">{trainer.trainerName}</p><p className="text-xs font-medium text-stone-600">{trainer.activeMembers} active members · {trainer.pendingPtSessions} pending</p></div></div><div className="text-right"><p className="font-mono text-sm font-bold text-stone-900">{trainer.completedPtSessions}</p><p className="text-xs uppercase tracking-wider text-stone-500">completed</p></div></div>)}</div>;
}
...
function SalesFunnel({ data }: { data: SalesFunnel | undefined }) {
 ...
 return <div className="space-y-5"><div className="flex items-center justify-between"><div><p className="text-xs font-black uppercase tracking-[.18em] text-stone-500">Conversion</p><p className="mt-1 text-3xl font-black tracking-tight text-stone-950">{conversion}%</p></div><div className="flex size-16 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg shadow-emerald-500/25"><Target className="size-6" aria-hidden="true" /></div></div><div className="h-3 overflow-hidden rounded-full bg-stone-100"><div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-cyan-400" style={{ width: `${conversion}%` }} /></div><div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-2xl bg-emerald-50 p-3 ring-1 ring-emerald-100"><p className="text-lg font-black text-emerald-700">{won}</p><p className="text-xs uppercase tracking-wider text-stone-600">Won</p></div><div className="rounded-2xl bg-rose-50 p-3 ring-1 ring-rose-100"><p className="text-lg font-black text-rose-700">{lost}</p><p className="text-xs uppercase tracking-wider text-stone-600">Lost</p></div><div className="rounded-2xl bg-sky-50 p-3 ring-1 ring-sky-100"><p className="text-lg font-black text-sky-700">{data.totalLeads}</p><p className="text-xs uppercase tracking-wider text-stone-600">Total</p></div></div><div className="border-t border-stone-100 pt-4"><div className="flex items-center justify-between text-xs"><span className="font-medium text-stone-600">Follow-up completion</span><span className="font-mono font-bold text-stone-900">{follow}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-stone-100"><div className="h-full rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-500" style={{ width: `${follow}%` }} /></div><p className="mt-2 text-xs font-medium text-stone-600">{data.followUps.completed} of {data.followUps.total} follow-ups completed</p></div></div>;
}
  ...
  <div className="relative -mx-2 min-h-full overflow-hidden pb-12 sm:-mx-3 lg:-mx-5">
   ...
   <div className="mx-auto flex max-w-[1680px] flex-col gap-8 px-2 sm:px-4 lg:px-6">
    ...
    <section aria-label="Finance and member health" className="grid gap-5 xl:grid-cols-[1.55fr_0.9fr]">
     <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-violet-50/80 via-white to-cyan-50/60 pb-4"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white shadow-lg"><BarChart3 className="size-5" aria-hidden="true" /></span><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Revenue trajectory</CardTitle></div></div><Badge variant="secondary" className="rounded-full bg-violet-50 text-violet-700 ring-1 ring-violet-100">{months}M view</Badge></div></CardHeader><CardContent className="p-5"><RevenueChart data={trend.data} /></CardContent></Card>
     <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-cyan-50/80 via-white to-blue-50/60 pb-4"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-lg"><Users className="size-5" aria-hidden="true" /></span><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Member status mix</CardTitle></div></div></CardHeader><CardContent className="p-5"><StatusBreakdown data={status.data} /></CardContent></Card>
    </section>
    ...
    <section aria-labelledby="intel-lifecycle">
     ...
     <div className="grid gap-5 xl:grid-cols-[1.55fr_0.9fr]">
      <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-emerald-50/80 via-white to-teal-50/60 pb-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg"><Activity className="size-5" aria-hidden="true" /></span><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Status &amp; renewal health</CardTitle></div></div><RefreshCw className="size-5 text-emerald-600" aria-hidden="true" /></div></CardHeader><CardContent className="p-5"><MembershipLifecycleCard data={lifecycle.data} loading={lifecycle.isLoading} /></CardContent></Card>
      <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="pb-2"><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-serif text-lg tracking-tight text-stone-950">Expiring soon</CardTitle></div><div className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-lg"><Calendar className="size-5" aria-hidden="true" /></div></div></CardHeader><CardContent className="pt-0"><div className="flex h-40 flex-col items-center justify-center rounded-xl bg-gradient-to-br from-amber-50/80 to-orange-50/50 text-center ring-1 ring-amber-100"><p className="font-mono text-4xl font-black text-amber-700 tabular-nums">{lifecycle.data?.expiringWithin30Days ?? "—"}</p><p className="mt-1 text-xs font-medium text-stone-600">memberships expire within 30 days</p></div></CardContent></Card>
     </div>
    </section>
    ...
    <section aria-labelledby="intel-queue">
     ...
     <div className="grid gap-5 lg:grid-cols-2">
      <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-amber-50/80 via-white to-orange-50/60"><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-serif text-lg tracking-tight text-stone-950">At-risk members</CardTitle></div><div className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-amber-500 to-orange-600 text-white shadow-lg"><AlertTriangle className="size-5" aria-hidden="true" /></div></div></CardHeader><CardContent className="pt-4"><AtRiskList members={atRisk.data} /></CardContent></Card>
      <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-violet-50/80 via-white to-fuchsia-50/60"><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-serif text-lg tracking-tight text-stone-950">Trainer workload</CardTitle></div><div className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white shadow-lg"><Dumbbell className="size-5" aria-hidden="true" /></div></div></CardHeader><CardContent className="pt-4"><TrainerList trainers={trainers.data} /></CardContent></Card>
     </div>
    </section>

    <section aria-label="Sales and acquisition" className="grid gap-5 xl:grid-cols-2">
     <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-emerald-50/80 via-white to-teal-50/60"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Sales funnel</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 text-white shadow-lg"><CreditCard className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><SalesFunnel data={sales.data} /></CardContent></Card>
     <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-violet-50/80 via-white to-fuchsia-50/60"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Source performance</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-violet-600 to-fuchsia-600 text-white shadow-lg"><Zap className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><SourcePerformance data={sources.data} /></CardContent></Card>
    </section>

    <section aria-label="Inventory and revenue quality" className="grid gap-5 lg:grid-cols-2">
     <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-cyan-50/80 via-white to-blue-50/60"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Inventory forecast</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-cyan-500 to-blue-600 text-white shadow-lg"><Package className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><InventoryList items={inventory.data} /></CardContent></Card>
     <Card className="overflow-hidden border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl"><CardHeader className="border-b border-stone-100/80 bg-gradient-to-r from-rose-50/80 via-white to-orange-50/60"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-serif text-xl tracking-tight text-stone-950">Revenue quality</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-gradient-to-br from-rose-500 to-orange-500 text-white shadow-lg"><WalletCards className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-stone-50 p-4 ring-1 ring-stone-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Gross revenue</p><p className="mt-1 text-xl font-black text-stone-950">{formatMoney(revenueRow?.grossRevenue)}</p></div><div className="rounded-2xl bg-rose-50/70 p-4 ring-1 ring-rose-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Refunded</p><p className="mt-1 text-xl font-black text-rose-600">{formatMoney(revenueRow?.refunded)}</p></div><div className="rounded-2xl bg-violet-50/70 p-4 ring-1 ring-violet-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Membership revenue</p><p className="mt-1 text-xl font-black text-stone-950">{formatMoney(revenueRow?.membershipRevenue)}</p></div><div className="rounded-2xl bg-cyan-50/70 p-4 ring-1 ring-cyan-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Other revenue</p><p className="mt-1 text-xl font-black text-stone-950">{formatMoney(revenueRow?.otherRevenue)}</p></div></div>{revenue.data?.notComputable?.length ? <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2"><AlertTriangle className="size-4 text-amber-600" aria-hidden="true" /><p className="text-xs font-bold text-amber-800">Data quality notes</p></div><div className="mt-2 space-y-1">{revenue.data.notComputable.map((item) => <p key={item.key} className="text-xs font-medium text-amber-700"><span className="font-bold">{item.key}:</span> {item.reason}</p>)}</div></div> : null}</CardContent></Card>
    </section>
   </div>
  </div>
 );
