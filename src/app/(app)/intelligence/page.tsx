...
function SectionHeader({ title, action }: { title: string; action?: React.ReactNode }) {
 ...
 <div>
 <h2 className="mt-1 font-semibold text-2xl font-semibold tracking-tight text-stone-950">{title}</h2>
 </div>
 {action}
 </div>
 );
...
function AtRiskList({ members }: { members: AtRiskMember[] | undefined }) {
 ...
 return <div className="flex flex-col gap-2">{members.slice(0, 6).map((member) => <Link key={member.id} href={`/members/${member.id}`} className="group flex min-h-11 items-center justify-between gap-3 rounded-xl border border-stone-100 bg-card p-3 transition hover:-translate-y-0.5 hover:border-amber-200 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-amber-600"><div className="flex min-w-0 items-center gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-amber-500 text-white shadow-md"><AlertTriangle className="size-4" aria-hidden="true" /></div><div className="min-w-0"><p className="truncate text-sm font-bold text-stone-900">{member.firstName} {member.lastName}</p><p className="text-xs font-medium text-stone-600">{member.neverCheckedIn ? "Never checked in" : `${member.daysSinceLastVisit} days since last visit`}</p></div></div><ChevronRight className="size-4 shrink-0 text-stone-300 transition group-hover:translate-x-0.5 group-hover:text-stone-700" aria-hidden="true" /></Link>)}{members.length > 6 ? <Link href="/members?filter=at-risk" className="flex min-h-11 items-center justify-center gap-1 pt-2 text-xs font-extrabold text-violet-700 hover:text-violet-900">View all {members.length} at-risk members <ArrowRight className="size-3.5" aria-hidden="true" /></Link> : null}</div>;
}
...
export default function IntelligencePage() {
 ...
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40 pb-4"><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-lg bg-violet-600 text-white shadow-lg"><BarChart3 className="size-5" aria-hidden="true" /></span><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Revenue trajectory</CardTitle></div></div><Badge variant="secondary" className="rounded-full bg-violet-50 text-violet-700 ring-1 ring-violet-100">{months}M view</Badge></div></CardHeader><CardContent className="p-5"><RevenueChart data={trend.data} /></CardContent></Card>
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40 pb-4"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-lg bg-cyan-500 text-white shadow-lg"><Users className="size-5" aria-hidden="true" /></span><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Member status mix</CardTitle></div></div></CardHeader><CardContent className="p-5"><StatusBreakdown data={status.data} /></CardContent></Card>
 </section>
 ...
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40 pb-4"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><span className="flex size-11 items-center justify-center rounded-lg bg-emerald-500 text-white shadow-lg"><Activity className="size-5" aria-hidden="true" /></span><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Status &amp; renewal health</CardTitle></div></div><RefreshCw className="size-5 text-emerald-600" aria-hidden="true" /></div></CardHeader><CardContent className="p-5"><MembershipLifecycleCard data={lifecycle.data} loading={lifecycle.isLoading} /></CardContent></Card>
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="pb-2"><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-semibold text-lg tracking-tight text-stone-950">Expiring soon</CardTitle></div><div className="flex size-11 items-center justify-center rounded-lg bg-amber-500 text-white shadow-lg"><Calendar className="size-5" aria-hidden="true" /></div></div></CardHeader><CardContent className="pt-0"><div className="flex h-40 flex-col items-center justify-center rounded-xl bg-muted/40 text-center ring-1 ring-amber-100"><p className="font-mono text-4xl font-black text-amber-700 tabular-nums">{lifecycle.data?.expiringWithin30Days ?? "—"}</p><p className="mt-1 text-xs font-medium text-stone-600">memberships expire within 30 days</p></div></CardContent></Card>
 </div>
 </section>
 ...
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40"><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-semibold text-lg tracking-tight text-stone-950">At-risk members</CardTitle></div><div className="flex size-11 items-center justify-center rounded-lg bg-amber-500 text-white shadow-lg"><AlertTriangle className="size-5" aria-hidden="true" /></div></div></CardHeader><CardContent className="pt-4"><AtRiskList members={atRisk.data} /></CardContent></Card>
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40"><div className="flex items-center justify-between gap-3"><div><CardTitle className="font-semibold text-lg tracking-tight text-stone-950">Trainer workload</CardTitle></div><div className="flex size-11 items-center justify-center rounded-lg bg-violet-600 text-white shadow-lg"><Dumbbell className="size-5" aria-hidden="true" /></div></div></CardHeader><CardContent className="pt-4"><TrainerList trainers={trainers.data} /></CardContent></Card>
 </div>
 </section>

 <section aria-label="Sales and acquisition" className="grid gap-5 xl:grid-cols-2">
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Sales funnel</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-emerald-500 text-white shadow-lg"><CreditCard className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><SalesFunnel data={sales.data} /></CardContent></Card>
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Source performance</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-violet-600 text-white shadow-lg"><Zap className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><SourcePerformance data={sources.data} /></CardContent></Card>
 </section>

 <section aria-label="Inventory and revenue quality" className="grid gap-5 lg:grid-cols-2">
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Inventory forecast</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-cyan-500 text-white shadow-lg"><Package className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><InventoryList items={inventory.data} /></CardContent></Card>
 <Card className="overflow-hidden border-white/90 bg-card shadow-sm shadow-violet-900/5 "><CardHeader className="border-b border-stone-100/80 bg-muted/40"><div className="flex items-center justify-between gap-3"><div><CardTitle className="mt-1 font-semibold text-xl tracking-tight text-stone-950">Revenue quality</CardTitle></div><span className="flex size-11 items-center justify-center rounded-lg bg-rose-500 text-white shadow-lg"><WalletCards className="size-5" aria-hidden="true" /></span></div></CardHeader><CardContent className="p-5"><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-lg bg-stone-50 p-4 ring-1 ring-stone-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Gross revenue</p><p className="mt-1 text-xl font-black text-stone-950">{formatMoney(revenueRow?.grossRevenue)}</p></div><div className="rounded-lg bg-rose-50/70 p-4 ring-1 ring-rose-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Refunded</p><p className="mt-1 text-xl font-black text-rose-600">{formatMoney(revenueRow?.refunded)}</p></div><div className="rounded-lg bg-violet-50/70 p-4 ring-1 ring-violet-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Membership revenue</p><p className="mt-1 text-xl font-black text-stone-950">{formatMoney(revenueRow?.membershipRevenue)}</p></div><div className="rounded-lg bg-cyan-50/70 p-4 ring-1 ring-cyan-100"><p className="text-xs font-bold uppercase tracking-wider text-stone-500">Other revenue</p><p className="mt-1 text-xl font-black text-stone-950">{formatMoney(revenueRow?.otherRevenue)}</p></div></div>{revenue.data?.notComputable?.length ? <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-4"><div className="flex items-center gap-2"><AlertTriangle className="size-4 text-amber-600" aria-hidden="true" /><p className="text-xs font-bold text-amber-800">Data quality notes</p></div><div className="mt-2 space-y-1">{revenue.data.notComputable.map((item) => <p key={item.key} className="text-xs font-medium text-amber-700"><span className="font-bold">{item.key}:</span> {item.reason}</p>)}</div></div> : null}</CardContent></Card>
 </section>
 </div>
 </div>
 );
