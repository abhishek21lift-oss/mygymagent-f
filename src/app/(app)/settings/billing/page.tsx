...
export default function PlatformBillingPage() {
 ...
 (async () => {
 try {
 const [p,u]=await Promise.all([api.get<Plan[]>("/platform-billing/plans"),api.get<Usage>("/platform-billing/usage")]);
 if (active) { setPlans(p); setUsage(u); }
 } catch (e) {
 if (active) toast.error(e instanceof Error ? e.message : "Billing could not be loaded");
 }
 })();
 return () => { active = false; };
 }, []);
