...
function TestSendCard({ canManage, orgCountry }: { canManage: boolean; orgCountry: string | null }) {
 ...
 {result && (
 <div className="flex flex-wrap items-center gap-2 rounded-lg border border-emerald-200 bg-emerald-50/70 p-3 dark:border-emerald-400/20 dark:bg-emerald-500/10">
...
function DangerZoneCard({ canManage }: { canManage: boolean }) {
 ...
 return (
 <Card className="overflow-hidden border-rose-200 bg-rose-50/40 shadow-sm shadow-rose-900/5 dark:border-rose-400/20 dark:bg-rose-500/5">
 <CardHeader className="border-b border-rose-100 bg-muted/40 dark:border-rose-400/10 dark:from-rose-500/10 dark:via-transparent dark:to-orange-500/10">
 <div className="flex items-start gap-3">
 ...
 <Button variant="destructive" disabled={!canManage} className="min-h-11 rounded-lg">
 Disconnect WhatsApp
 </Button>
 </DialogTrigger>
 <DialogContent>
 ...
 <DialogFooter>
 <Button variant="outline" onClick={() => handleOpenChange(false)} disabled={disconnect.isPending} className="min-h-11 rounded-lg">
 Cancel
 </Button>
 <Button variant="destructive" onClick={handleConfirm} disabled={disconnect.isPending} className="min-h-11 rounded-lg">
