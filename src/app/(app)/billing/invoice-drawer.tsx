   ...
   <DialogContent className="left-auto right-0 top-0 flex h-dvh w-full translate-x-0 translate-y-0 flex-col gap-4 overflow-y-auto rounded-none sm:max-w-lg">
    ...
    {detail.isLoading ? (
     <div className="flex flex-col gap-3" aria-label="Loading invoice">
      {[1, 2, 3].map((i) => <div key={i} className="h-16 animate-pulse rounded-lg bg-stone-200/70 dark:bg-white/10" />)}
     </div>
     ...
     <div className="flex flex-col gap-5">
      ...
      <div>
       <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Lines</h3>
       ...
       {(invoice.taxBreakup?.length ?? 0) > 0 && (
        <div className="mt-2 rounded-lg border border-stone-200/70 p-3 text-sm dark:border-white/10">
       ...
       {invoice.dunningAttempts.length === 0 ? (
        ...
        <ol className="flex flex-col gap-2">
         {invoice.dunningAttempts.map((d, i) => (
          <li key={i} className="flex items-center justify-between rounded-lg border border-stone-200/70 p-3 text-sm dark:border-white/10">
           <div>
      ...
      {unconfigured && (
       <div className="rounded-lg border border-amber-200/70 bg-amber-50/70 p-3 text-sm dark:border-amber-900/50 dark:bg-amber-950/30">
