...
function RefundCell({ payment }: { payment: Payment }) {
  ...
  if (payment.status === "REFUNDED") return <span className="text-xs text-muted-foreground">Fully refunded</span>;
...
const columns: ColumnDef<Payment>[] = [
    ...
    cell: ({ row }) => {
      ...
      return member ? (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{row.original.note ?? "Payment"}</p>
        </div>
      ) : "—";
    },
  },
  {
    header: "Amount",
    accessorKey: "amount",
    cell: ({ row }) => <span className="font-semibold tabular-nums">{row.original.currency} {row.original.amount}</span>,
  },
  {
    header: "Method",
    accessorKey: "method",
...
export default function BillingPage() {
  ...
  return (
    <div className="flex flex-col gap-7">
      <PageHeader
        title="Finance OS"
        description="Track collections, refunds and payment activity with financial context."
        actions={hasPermission("payments.create") ? <RecordPaymentDialog /> : undefined}
      />
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-background/70 px-3 py-1.5 text-xs font-semibold text-primary">
              <Sparkles className="size-3.5" /> Finance intelligence
            </div>
            <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Know where the money moved.</h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">
              A clean operating view for collections, refunds and payment activity. No fabricated aggregates—only data available to the current finance API.
            </p>
          </div>
          <Button asChild variant="outline" className="rounded-xl bg-background/60">
            <a href="/ai"><Sparkles className="size-4" /> Ask AI about revenue</a>
          </Button>
        </div>
      </section>
        ...
        <CardHeader className="border-b bg-muted/20">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">Current transactions and refund actions.</p>
            </div>
  ...
  return (
    <Card className="border-0 shadow-sm ring-1 ring-border/70">
      <CardContent className="p-4">
        ...
        {loading ? <div className="mt-4 h-7 w-24 animate-pulse rounded bg-muted" /> : <p className="mt-4 text-2xl font-semibold tracking-tight tabular-nums">{value}</p>}
  ...
  return (
    <Card className="border-0 shadow-sm ring-1 ring-border/70">
      <CardContent className="p-4">
        ...
        <span className="mt-3 inline-flex items-center gap-1 text-[11px] font-medium text-primary">Planned next <ArrowRight className="size-3" /></span>
      </CardContent>
    </Card>
  );
}
