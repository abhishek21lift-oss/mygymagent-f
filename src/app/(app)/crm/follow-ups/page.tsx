 ...
 return (
  <div className="relative -mx-2 min-h-full overflow-hidden pb-12 sm:-mx-3 lg:-mx-5">
   ...
   <div className="mx-auto flex max-w-[1680px] flex-col gap-8 px-2 sm:px-4 lg:px-6">
    ...
    <section aria-labelledby="followups-list" className="overflow-hidden rounded-xl border border-white/90 bg-white/88 shadow-xl shadow-violet-900/5 backdrop-blur-xl">
     ...
     <div className="space-y-3 p-4 sm:p-5">
      {query.isLoading && (
       <div className="space-y-2" aria-label="Loading follow-ups">
        {[1, 2, 3].map((i) => (
         <div key={i} className="h-[86px] animate-pulse rounded-xl bg-gradient-to-r from-stone-100 to-stone-50" />
        ))}
       </div>
      )}
      {query.isError && (
       <div role="alert" className="rounded-xl border border-rose-200 bg-gradient-to-r from-rose-50 to-orange-50 p-6 text-center">
      ...
      {!query.isLoading && !query.isError && rows.map((row) => (
       <div
        key={row.id}
        className="relative flex flex-col gap-4 overflow-hidden rounded-xl border border-white/90 bg-white/80 p-4 shadow-sm transition hover:-translate-y-px hover:shadow-md sm:flex-row sm:items-center sm:justify-between"
       >
        <span
      ...
      {!query.isLoading && !query.isError && rows.length === 0 && (
       <div className="rounded-xl border border-dashed border-blue-200 bg-gradient-to-br from-blue-50/60 to-cyan-50/40 p-10 text-center">
...
function Metric({ icon: Icon, label, value, hint, tone }: { icon: typeof ListChecks; label: string; value: number; hint?: string; tone: StatTone }) {
 ...
 return (
  <div className={`group relative overflow-hidden rounded-xl border border-white/90 bg-white/85 p-5 shadow-[0_20px_60px_-38px_rgba(79,70,229,.35)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 ${t.ring}`}>
   ...
   <span className={`relative flex size-14 items-center justify-center rounded-lg bg-gradient-to-br text-white shadow-lg ${t.tile}`}>
