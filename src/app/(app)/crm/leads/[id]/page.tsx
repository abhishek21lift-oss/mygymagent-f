...
export default function Lead360Page({ params }: { params: { id: string } }) {
  ...
  async function save(values: CreateLeadInput) {
    try {
      await update.mutateAsync({ id: currentLead.id, input: values })
  ...
  async function changeStatus(value: string) {
    try {
      await statusMutation.mutateAsync({ id: currentLead.id, status: value as LeadStatus })
  ...
  async function createFollowUp(values: CreateFollowUpInput) {
    try {
      await addFollowUp.mutateAsync({ leadId: currentLead.id, input: { ...values, dueAt: new Date(`${values.dueAt}T12:00:00`).toISOString() } })
  ...
  async function finishFollowUp(followUpId: string) {
    try {
      await completeFollowUp.mutateAsync({ leadId: currentLead.id, followUpId })
    ...
    <div className="flex flex-col gap-6 pb-8">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <Card className="border-0 shadow-sm ring-1 ring-border/70">
          ...
          <CardContent className="p-6"><Form {...form}><form onSubmit={form.handleSubmit(save)} className="grid gap-5 sm:grid-cols-2">
      ...
      <Card className="border-0 shadow-sm ring-1 ring-border/70">
        ...
        <CardContent className="space-y-5"><Form {...followUpForm}><form onSubmit={followUpForm.handleSubmit(createFollowUp)} className="grid gap-3 sm:grid-cols-[180px_1fr_auto]">
          ...
          <div className="space-y-3">{currentLead.followUps?.map((followUp) => <div key={followUp.id} className="flex flex-col gap-3 rounded-2xl border bg-background/50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><Badge variant={followUp.completedAt ? "secondary" : "warning"}>{followUp.completedAt ? "Completed" : "Open"}</Badge><span className="text-sm font-medium">{new Date(followUp.dueAt).toLocaleDateString()}</span></div><p className={followUp.completedAt ? "mt-2 text-sm text-muted-foreground line-through" : "mt-2 text-sm"}>{followUp.note}</p></div>{!followUp.completedAt && <Button size="sm" variant="outline" onClick={() => finishFollowUp(followUp.id)} disabled={completeFollowUp.isPending}><Check className="size-3.5" /> Done</Button>}</div>)}
            {(!currentLead.followUps || currentLead.followUps.length === 0) && <div className="rounded-2xl border border-dashed p-6 text-center text-sm text-muted-foreground">No follow-ups yet. Schedule the next sales action above.</div>}
          </div>
        </CardContent>
      </Card>
    </div>
...
function Metric({ icon: Icon, label, value, tone }: { icon: typeof Users; label: string; value: string | number; tone: "violet" | "rose" | "amber" | "cyan" }) {
