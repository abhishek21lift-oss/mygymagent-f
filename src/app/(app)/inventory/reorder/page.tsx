...
export default function ReorderPage(){
 ...
 <div className="flex flex-wrap gap-3"><select value={branchId} onChange={e=>setBranchId(e.target.value)} className="h-10 rounded-xl border border-stone-200 bg-card px-3 text-sm font-semibold"><option value="">Organization stock</option>{(branches.data?.items??[]).map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
