...
export default function ReportsPage(){
 ...
 return <InventoryResourceShell title="Inventory Reports" description="Operational snapshot assembled from the authoritative inventory endpoints.">
  ...
  ].map(([k,v])=><div key={String(k)} className="rounded-2xl border bg-white/85 p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wider text-stone-500">{k}</p><p className="mt-2 text-2xl font-black tabular-nums">{String(v)}</p></div>)}</div>
