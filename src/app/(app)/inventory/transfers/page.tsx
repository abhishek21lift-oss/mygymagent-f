...
export default function TransfersPage(){
 ...
 <DataState isLoading={q.isPending} isError={q.isError} onRetry={()=>void q.refetch()} errorMessage="Could not load transfers." isEmpty={(q.data??[]).length===0} emptyTitle="No transfers yet" emptyDescription="Stock moved between branches will appear here."><div className="grid gap-3">{(q.data??[]).map(t=><div key={t.id} className="rounded-lg border bg-card p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="font-black">{t.number}</h3><p className="text-sm text-stone-500">{t.status}</p></div>{t.status==="DRAFT"&&<Button onClick={()=>act(()=>ship.mutateAsync(t.id),"Transfer shipped")}>Ship</Button>}{t.status==="IN_TRANSIT"&&<Button onClick={()=>act(()=>receive.mutateAsync(t.id),"Transfer received")}>Receive</Button>}</div><div className="mt-4 text-sm text-stone-600">{t.fromBranch?.name||"Source"} → {t.toBranch?.name||"Destination"} · {t.items?.length||0} item(s)</div></div>)}{!q.isLoading&&!(q.data?.length)&&<div className="rounded-lg border bg-white p-10 text-center text-stone-500">No transfers.</div>}</div></DataState>
