...
export default function MemberPortal(){
 ...
 return <main className="min-h-svh bg-muted/40 p-4 sm:p-8"><div className="mx-auto max-w-5xl space-y-6">
 <Card className="border-0 shadow-lg"><CardHeader><div className="flex items-center gap-3"><div className="rounded-lg bg-primary/10 p-3"><ShieldCheck className="size-6"/></div><div><CardTitle>Member Portal</CardTitle><p className="text-sm text-muted-foreground">Secure single-use access to your gym account.</p></div></div></CardHeader><CardContent><div className="flex flex-col gap-3 sm:flex-row"><Input value={token} onChange={e=>setToken(e.target.value)} placeholder="Paste portal token"/><Button className="sm:w-40" onClick={()=>void load(token)} disabled={loading}>{loading?<Loader2 className="mr-2 size-4 animate-spin"/>:null}{loading?"Opening...":"Open portal"}</Button></div>{error&&<div className="mt-3 flex items-center gap-2 rounded-xl border border-destructive/30 bg-destructive/5 p-3 text-sm text-destructive"><XCircle className="size-4"/>{error}</div>}</CardContent></Card>
