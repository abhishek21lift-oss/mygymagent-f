...
export function InventoryNav() {
  ...
  return <nav aria-label="Inventory navigation" className="flex gap-2 overflow-x-auto pb-1">
    {items.map(([href,label,Icon]) => {
      ...
      return <Link key={href} href={href} className={`inline-flex shrink-0 items-center gap-2 rounded-xl border px-3 py-2 text-xs font-extrabold transition ${active ? "border-amber-300 bg-amber-50 text-amber-900 shadow-sm" : "border-stone-200 bg-white/75 text-stone-600 hover:border-amber-200 hover:bg-amber-50/60"}`}>
