...
export function Topbar({ onOpenMobileNav, sidebarCollapsed = false, onToggleSidebar }: { onOpenMobileNav: () => void; sidebarCollapsed?: boolean; onToggleSidebar?: () => void }) {
 ...
 return (
  <header className="relative z-40 shrink-0 border-b bg-card/90 backdrop-blur">
   <div className="mx-auto flex min-h-16 w-full items-center gap-2 px-3 py-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] sm:px-4 sm:pt-2">
    <div className="flex shrink-0 items-center gap-1">
     <Button variant="ghost" size="icon" className="hidden min-h-10 min-w-10 md:inline-flex" onClick={onToggleSidebar} aria-label={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"} title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}>
      {sidebarCollapsed ? <ChevronsRight className="size-4" aria-hidden="true" /> : <ChevronsLeft className="size-4" aria-hidden="true" />}
     </Button>
     <Button variant="ghost" size="icon" className="min-h-10 min-w-10 md:hidden" onClick={onOpenMobileNav} aria-label="Open navigation">
    ...
    <div className="flex shrink-0 items-center gap-1">
     <Button variant="ghost" size="icon" className="min-h-10 min-w-10" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} aria-label="Toggle theme">
      <Sun className="size-4 dark:hidden" aria-hidden="true" /><Moon className="hidden size-4 dark:block" aria-hidden="true" />
     </Button>
     <DropdownMenu>
      <DropdownMenuTrigger asChild>
       <Button variant="ghost" className="min-h-10 gap-2 px-2" aria-label={user ? `Account: ${user.firstName} ${user.lastName}` : "Account"}>
        <Avatar className="size-8"><AvatarFallback className="text-xs font-bold">{user ? initials(user.firstName, user.lastName) : <UserIcon className="size-4" aria-hidden="true" />}</AvatarFallback></Avatar>
      ...
      <DropdownMenuContent align="end" className="w-64 p-1.5">
       ...
       <DropdownMenuItem variant="destructive" className="min-h-11" onClick={handleLogout}><LogOut aria-hidden="true" />Log out</DropdownMenuItem>
      </DropdownMenuContent>
     </DropdownMenu>
    </div>
   </div>
