...
function TabLink({ tab, active }: { tab: TabItem; active: boolean }) {
 ...
 return (
  <Link
   ...
   className={cn(
    "relative flex min-h-14 flex-1 flex-col items-center justify-center gap-0.5 touch-manipulation rounded-lg transition-colors focus-visible:outline-2 focus-visible:outline-ring",
    active
  ...
  >
   <div className="relative mx-auto flex max-w-lg items-stretch justify-between gap-1 px-2 py-1.5">
    ...
    <Link
     ...
     className="relative -mt-6 flex flex-1 touch-manipulation flex-col items-center justify-end gap-1 pb-1 focus-visible:outline-2 focus-visible:outline-ring rounded-lg"
    >
     <span
      className={cn(
       "flex size-14 items-center justify-center rounded-full border shadow-md transition-transform",
       aiActive
     ...
     <span
      className={cn(
       "rounded-full border px-2 py-0.5 text-xs font-semibold tracking-tight",
       aiActive ? "border-primary bg-primary/10 text-primary" : "border-border bg-muted text-muted-foreground",
      )}
     >
      AI
     </span>
    ...
    <button
     type="button"
     onClick={onOpenMore}
     className="flex min-h-14 flex-1 touch-manipulation flex-col items-center justify-center gap-0.5 rounded-lg font-medium text-muted-foreground transition-colors hover:text-foreground focus-visible:outline-2 focus-visible:outline-ring"
