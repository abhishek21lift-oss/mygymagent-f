  ...
  return (
    <Link
      ...
      className={cn(
        ...
        nested ? "ml-4 rounded-lg px-3 py-1.5 text-xs" : "rounded-xl px-3 py-2.5 text-sm",
        active
    ...
    >
      <Icon
        className={cn(
          ...
          active ? "text-primary-foreground" : item.accent === "ai" ? "text-primary" : "text-sidebar-foreground/50 group-hover:text-sidebar-foreground",
        )}
      />
...
export function SidebarNav({ className }: { className?: string }) {
  ...
  return (
    <nav className={cn("flex h-full flex-col bg-sidebar text-sidebar-foreground p-3", className)}>
      <Link href="/dashboard" className="mb-6 flex items-center gap-2.5 rounded-xl px-2 py-1.5">
        <Image src="/logo-mark.webp" alt="" width={36} height={36} className="size-9 shrink-0 object-contain" priority />
        <div className="min-w-0">
          <div className="truncate text-[15px] font-bold tracking-tight">MyGymAgent</div>
          <div className="text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground">Gym OS</div>
        </div>
      </Link>
      ...
      <div className="flex flex-1 flex-col gap-1 overflow-y-auto">
            ...
            <div key={item.href}>
              ...
              {active && children.length > 0 && (
                <div className="mt-1 mb-2 space-y-0.5 border-l border-primary/15 pl-1">
                  {children.map((child) => (
                    <NavLink key={child.href} item={child} active={pathname.startsWith(child.href)} nested />
                  ))}
                </div>
              )}
            </div>
        ...
        {visibleComingSoon.length > 0 && (
          <div className="mt-5">
            <div className="mb-2 px-3 text-[10px] font-semibold uppercase tracking-[0.16em] text-sidebar-foreground/40">Coming soon</div>
            <div className="space-y-1">
              {visibleComingSoon.map((item) => <NavLink key={item.href} item={item} active={pathname.startsWith(item.href)} />)}
            </div>
          </div>
        )}
      </div>
