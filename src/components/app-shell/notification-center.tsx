...
const filters = [
  { key: "", label: "All" }, { key: "unread", label: "Unread" },
  { key: "MEMBERS", label: "Members" }, { key: "MEMBERSHIPS", label: "Memberships" },
  { key: "PAYMENTS", label: "Payments" }, { key: "CRM", label: "CRM" },
...
function NotificationRow({ item, onRead, onUnread, onOpen, onArchive, onDelete, onSnooze }: {
  ...
  return (
    <div className={cn("group relative border-b border-border/50 px-4 py-3 transition-colors last:border-b-0", unread ? "bg-primary/[0.045]" : "bg-transparent")}>
      ...
      <div className="flex items-start gap-3">
        ...
        <button type="button" className="min-w-0 flex-1 text-left outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" onClick={() => onOpen(item)}>
          ...
          <span className="mt-2 flex items-center gap-1.5">
            <Badge variant={item.priority === "CRITICAL" ? "destructive" : item.priority === "HIGH" ? "secondary" : "outline"} className="h-5 rounded-md px-1.5 text-[9px]">{priorityLabel(item.priority)}</Badge>
            <span className="text-[9px] font-semibold uppercase tracking-wider text-muted-foreground">{item.category}</span>
          </span>
        </button>
        <div className="flex shrink-0 items-center gap-0.5 opacity-70 transition-opacity group-hover:opacity-100">
          <Button type="button" variant="ghost" size="icon" className="size-7 rounded-lg" onClick={() => unread ? onRead(item.id) : onUnread(item.id)} aria-label={unread ? "Mark notification as read" : "Mark notification as unread"} title={unread ? "Mark as read" : "Mark as unread"}>
...
export function NotificationCenter() {
  ...
  const snoozeMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (e) => toast.error(e instanceof ApiError ? e.message : "Failed to snooze notification"),
  })
  const readAllMutation = useMutation({ mutationFn: markAllNotificationsRead, onSuccess: invalidate, onError: (e) => toast.error(e instanceof ApiError ? e.message : "Failed to mark notifications as read") })
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative min-h-10 min-w-10 rounded-xl border border-transparent hover:border-border/60 hover:bg-card/70" aria-label={unreadCount > 0 ? `Notifications, ${unreadCount} unread` : "Notifications"} title="Notifications">
      ...
      <PopoverContent align="end" sideOffset={10} className="w-[min(34rem,calc(100vw-1rem))] overflow-hidden rounded-2xl border-border/60 bg-background/95 p-0 shadow-2xl backdrop-blur-2xl">
        <div className="border-b border-border/60 px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1">
              <Button variant="ghost" size="icon" className="size-9 rounded-lg" onClick={() => query.refetch()} aria-label="Refresh notifications" title="Refresh"><RefreshCw className={cn("size-4", query.isFetching && "animate-spin")} aria-hidden="true" /></Button>
              <Button variant="ghost" size="icon" className="size-9 rounded-lg" disabled={unreadCount === 0 || readAllMutation.isPending} onClick={() => readAllMutation.mutate()} aria-label="Mark all notifications as read" title="Mark all as read"><CheckCheck className="size-4" aria-hidden="true" /></Button>
              <Button variant="ghost" size="icon" className="size-9 rounded-lg" onClick={() => { setOpen(false); router.push("/settings/notifications") }} aria-label="Notification settings" title="Notification settings"><Settings2 className="size-4" aria-hidden="true" /></Button>
            </div>
          </div>
          <form onSubmit={applySearch} className="mt-3 flex gap-2">
            <div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" aria-hidden="true" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search notifications..." aria-label="Search notifications" className="h-9 rounded-xl pl-9" /></div>
            <Button type="submit" size="sm" variant="outline" className="h-9 rounded-xl">Search</Button>
          </form>
          <div className="mt-3 flex gap-1 overflow-x-auto pb-0.5" aria-label="Notification filters"><Filter className="mt-2 size-3.5 shrink-0 text-muted-foreground" aria-hidden="true" />{filters.map((item) => <Button key={item.key} type="button" size="sm" variant={filter === item.key ? "default" : "ghost"} className="h-7 shrink-0 rounded-lg px-2.5 text-[11px]" onClick={() => setFilter(item.key)}>{item.label}</Button>)}</div>
        </div>
