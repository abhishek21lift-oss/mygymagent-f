...
function NotificationRow({
  ...
  return (
    <div
      className={cn(
        "group relative border-b border-border/50 px-4 py-3.5 transition-colors last:border-b-0",
    ...
    >
      ...
      {unread && (
        <Button
          ...
          variant="ghost"
          size="sm"
          className="mt-2 ml-11 h-8 rounded-lg px-2 text-xs opacity-70 hover:opacity-100"
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent
        align="end"
        sideOffset={10}
        className="w-[min(25rem,calc(100vw-1.5rem))] overflow-hidden rounded-2xl border-border/60 bg-background/95 p-0 shadow-2xl backdrop-blur-2xl"
      >
        ...
        <div className="max-h-[min(32rem,65vh)] overflow-y-auto">
          {query.isPending ? (
            ...
            <div className="p-6 text-center">
              ...
              <Button variant="outline" size="sm" className="mt-4 rounded-xl" onClick={() => query.refetch()}>
                Try again
              </Button>
            </div>
          ) : visibleCount === 0 ? (
            <div className="p-8 text-center">
              ...
              <p className="mt-1 text-xs leading-5 text-muted-foreground">Important gym activity will appear here.</p>
            </div>
          ) : (
            query.data?.items.map((item) => (
              <NotificationRow key={item.id} item={item} onRead={(id) => readMutation.mutate(id)} onOpen={openNotification} />
            ))
          )}
        </div>
