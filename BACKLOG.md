...
| F-P0-4 | Handle the MFA challenge in login, and add the enrolment UI | ⬜ | The backend now ships TOTP 2FA (`mygymagent-b` B-P0-4). `POST /auth/login` can answer `{ mfaRequired: true, mfaToken, expiresIn }` **instead of** `{ user, accessToken }` — `src/app/(auth)/login/page.tsx` and `src/lib/auth/auth-context.tsx` currently assume the second shape, so an enrolled user would hit a broken login. **This is not yet breaking anyone: enrolment is opt-in and nobody can enrol until this UI exists** — but it must land before B-P0-10 makes MFA mandatory for privileged roles. Needs: (1) login branches on `mfaRequired` into a code prompt that POSTs `{ mfaToken, code }` to `/auth/mfa/verify` (same response shape as login, incl. the refresh cookie), accepting a recovery code in the same field; (2) a settings screen over `GET /auth/mfa`, `POST /auth/mfa/setup` (render `otpauthUri` as a QR — the backend deliberately returns the URI, not an image), `POST /auth/mfa/enable`, `POST /auth/mfa/disable`; (3) show the 10 recovery codes exactly once at enable time, with a copy/download affordance, since they are never retrievable again. |
