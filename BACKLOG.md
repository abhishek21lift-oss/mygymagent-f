...
| F-P0-7 | Per-page layout depth for the remaining routes | ⬜ | The redesign made every route share a canvas, a type scale, working dark mode and one metric component — but only Members and Home had their internal layout reconsidered. 17 pages still open with a section heading above a metric strip (the heading tells you four numbers are numbers), 15 decorative icon chips remain inside card headers, and several pages still order their sections by accretion rather than by what the operator needs first. This is per-page judgement, not a systemic fix: work through the highest-traffic routes (Sales, Attendance, Inventory, Payroll, Member detail) applying the pattern Members now uses — header, metric strip, one toolbar row, then the data. |
